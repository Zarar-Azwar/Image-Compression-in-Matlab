function[output,avglen,OrigSize,codeSize]= projectHuffmanEncoding(I)
    con=I;
    con=floor(con/5);
    Vector=con(:);
    ...
    [dict,avglen] = huffmandict(uniqueVal,prob);
    imgOneD = I(:) ;
    imgVal = imgOneD ;
    ...
    OrigSize=numel(de2bi(imgVal))/kb;
    disp(OrigSize) ;
    fprintf('Encoding size');
    codeSize=numel(encodedVal)/kb;
    disp(codeSize) ;
